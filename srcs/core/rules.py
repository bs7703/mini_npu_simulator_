...
def validate_tensor(data, spec, context) -> tuple[bool, str]:
    ...
    if not isinstance(data, list):
        if isinstance(data, target_type):
            return True, ""
        return False, const.ErrCode.TYPE_ERROR.format(required_type= target_type,user_type = type(data))
    if len(data) != size:
...
def validate_list(data, spec, context) -> tuple[bool, str]:
    if data in spec.get("enum", []):
        return True, ""
    return False, const.ErrCode.ENUM_ERROR.format(user_data = data, enum_list = spec.get("enum", []))
